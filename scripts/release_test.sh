...
# A repository whose releases come from another branch says so, and is held to
# it exactly as main is.
git init -q --bare -b master "$work/master.git"
repo="$work/master"
git clone -q "$work/master.git" "$repo" 2>/dev/null
git -C "$repo" config user.email test@example.com
git -C "$repo" config user.name test
...
refuses "a release from master where releases come from main" "main" "1.0.0"
if out="$(cd "$repo" && RELEASE_BRANCH=master RELEASE_CHECK=true "$release" 1.0.0 2>&1)" \
   && git -C "$work/master.git" rev-parse -q --verify refs/tags/v1.0.0 >/dev/null; then
  ok "a release from the branch RELEASE_BRANCH names is tagged and pushed to that branch"
else
  fail "releasing from master: $out"
fi
